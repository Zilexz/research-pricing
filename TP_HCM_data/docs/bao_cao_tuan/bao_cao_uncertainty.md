...
| `uncertainty/ve_chuoi_thoi_gian.py` | Giá theo thời gian: giá · sai số · coverage | `TT0` |
| **`tuan_4/00_TONG_HOP`** | ⭐ Bản tổng hợp một trang cả tuần 4 — đọc trước | `TK1`–`TK4` |
| `tuan_4/01_BA_KICH_BAN` | Ba kịch bản mentor vẽ trên chính đường giá (kèm `ve_ba_kich_ban.py` để chạy nhanh) | `TT5` `TT6` |
| `tuan_4/02_IMPROVE_HAY_GIAM_UNCERTAINTY` | Improve model hay giảm uncertainty · fail ở đâu | `QD1`–`QD3` |
| `tuan_4/03_CETERIS_PARIBUS_VA_CAUSALITY` | Giá phản ứng với từng yếu tố · model có hiểu cơ chế không | `PU1` `PU2` |
| `tuan_4/04_CAU_THANH_GIA` | Đủ bộ yếu tố cấu thành giá · trần thông tin của giá cơ bản | `CG1`–`CG3` |
