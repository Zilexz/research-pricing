...
**Nháy đúp vào `index.html`.** Không cần cài gì, không cần chạy server, không cần mạng —
dữ liệu nhúng sẵn trong file và thư viện Leaflet nằm trong `vendor/`.

> ⚠️ **Không có mạng thì demo vẫn chạy đủ**, chỉ mất **nền bản đồ đường phố** (ảnh nền lấy từ
> OpenStreetMap qua CARTO). Ba khu vực, tuyến đường, xe chạy, mọi biểu đồ và bảng đều bình thường.
>
> Bản `demo/` cũ tải Leaflet từ `unpkg.com`; mạng chặn CDN là `L.map()` nổ và **toàn bộ** script
> chết chứ không chỉ mất nền. Gói `demo_v2/` này bỏ hẳn phụ thuộc đó.
